...
if ($court_id > 0) {
    $stmt = $pdo->prepare("SELECT * FROM court_times WHERE court_id = ? ORDER BY time_slot");
    $stmt->execute([$court_id]);
    $times = $stmt->fetchAll();

    echo '<form method="POST" action="update_court_times.php">';
    echo '<input type="hidden" name="court_id" value="' . $court_id . '">';
